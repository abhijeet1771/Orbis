 ...
 */
export function normalizeIdentities(test: TestCaseResult, log?: { fixed: boolean }): TestCaseResult {
  if (!test.identities || test.identities.length === 0) {
    // No identities to normalize
    return test;
  }

  const normalizedIdentities: ExternalIdentity[] = [];
  let hasFixes = false;
 ...
 */
function normalizeSingleIdentity(identity: ExternalIdentity): ExternalIdentity | null {
 ...
 */
export function extractIdentities(test: TestCaseResult): ExternalIdentity[] {
  ...
  const uniqueIdentities = identities.filter((identity, index, arr) =>
 ...
 */
function extractFromAnnotations(annotations: Record<string, unknown>): ExternalIdentity[] {
  const identities: ExternalIdentity[] = [];
  ...
  for (const [key, value] of Object.entries(annotations)) {
    ...
    if (typeof value === 'string' && isValidSystem(key)) {
      ...
      for (const id of ids) {
        identities.push({
          system: key as ExternalIdentity['system'],
          id,
          source: 'annotation'
        });
      }
    ...
    if (key === 'type' && typeof value === 'string' && isValidSystem(value)) {
      ...
      if (typeof description === 'string') {
        ...
        for (const id of ids) {
          identities.push({
            system: value as ExternalIdentity['system'],
            id,
            source: 'annotation'
          });
        }
 ...
 */
function extractFromTags(tags: string[]): ExternalIdentity[] {
  const identities: ExternalIdentity[] = [];
  ...
  for (const tag of tags) {
    ...
    if (systemPrefixMatch) {
      ...
      for (const id of ids) {
        identities.push({
          system: system.toLowerCase() as ExternalIdentity['system'],
          id,
          source: 'tag'
        });
      }
 ...
 */
function extractFromTitle(title: string): ExternalIdentity[] {
  const identities: ExternalIdentity[] = [];
 ...
 */
function isValidSystem(system: string): system is ExternalIdentity['system'] {
